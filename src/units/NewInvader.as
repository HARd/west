package units 
{
	...
	public class NewInvader extends WUnit
	{
		...
		public function NewInvader(object:Object) 
		{
		...
		{			
			...
			do
			{
				...
				if (NewInvader.checkFreeNode(rX, rZ, findUnit)){
					findFreePos = true;
				}
		...
		public function setPosition ( object: Object):Object {
			...
			for (var count:int = 0; count < serchSet; ++count ) {
				...
				// check free node
				if (NewInvader.checkFreeNode(_x,_z,object))
					break;
			}
			if (count >= serchSet) {
		...
		public function onHide():void {
			...
			if ( shake )
			{
				...
				for each (var inv:NewInvader in invaders)
				{
					if ( inv != this ) {
						TweenLite.to(inv, 1, { alpha:0, onCompleteParams:[inv], onComplete:function (... args):void {
							var invader:NewInvader = args[0] as NewInvader;
