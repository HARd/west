package astar
{
	...
	public class AStar
	{
		...
		public function AStar( grid : Vector.<Vector.<AStarNodeVO>> ){
			_length = grid.length * grid.length + 1;
			_touched = new Vector.<AStarNodeVO>(_length, true);
			_grid = grid;
		...
		public function take(nodes:Vector.<AStarNodeVO>):void {
			for each(var node:AStarNodeVO in nodes) {
				for each(var neighbor:AStarNodeVO in node.neighbors) {
					if (neighbor == null)
						break;
					if (!neighbor.isWall){
						var index:int = neighbor.neighbors.indexOf(node);
						if (index != -1) {
		...
		public function free(nodes:Vector.<AStarNodeVO>):void {
			for each(var node:AStarNodeVO in nodes) {
				for each(var neighbor:AStarNodeVO in node.neighbors) {
					if (neighbor == null)
						break;
		...
		public function reload():void {
			...
			var rows:int = _grid.length;
			for (var i:int = 0; i < rows; i++){
				var row:Vector.<AStarNodeVO> = _grid[i];
				var nodes:uint = row.length;
				for (var j:int = 0; j < nodes; j++) {  
					var node:AStarNodeVO = row[j];
					
					node.neighbors = new Vector.<AStarNodeVO>();
					var _neighbors:* = getNeighbors(_grid, node);
					var l:int = _neighbors.length;
					for (var k:int = 0; k < l; k++) {
						
						var neighbor:AStarNodeVO = _neighbors[k];
