package wins.elements {
	...
	public class PurchaseItem extends Sprite {
		...
		public function PurchaseItem(sID:int, callback:Function, window:*, id:uint, doGlow:Boolean = false, noTitle:Boolean = false, noDesc:Boolean = false) {
			...
			if (App.data.storage[sID].type != 'Firework' && App.data.storage[sID].out != Techno.TECHNO && App.data.storage[sID].out != Ttechno.TECHNO) addChild(searchBttn);
		...
		private function onSearchOut(e:MouseEvent):void {
			...
			if (App.data.storage[sID].type == 'Firework') {
				...
				var fl:Boolean = ShopWindow.findMaterialSource(App.data.storage[sID].out);
			}
		}
		...
		{
			var units:Array = Map.findUnitsByType(['wigwam']);
			for each (var u:Wigwam in units) {
