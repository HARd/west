package wins {
	...
	public class RewardListB extends Sprite {
...
internal class RewardItem extends LayerX {
	...
	public function RewardItem(sID:uint, count:int, _callBack:Function, _size:int = 56, _preText:String = "", iconScale:Number = 0.74, marginTxtX:int = 0, marginTxtY:int = 0, hasBackingItem:Boolean = true, textIsRight:Boolean = false) {
		...
		nameItem = Window.drawText(String(App.data.storage[sID].title), {
			fontSize:32,
			color	:0x7a3a14,
			borderColor	:0xfaf7e6,
			width:140,
			textAlign:"center"
			//autoSize:"left"
		});
	...
	private function onLoad(data:Bitmap):void {
		...
		background = new Bitmap(new BitmapData(140, 140, true, 0xffffff));
		background.x = 0;
		background.y = 70;
		...
		shape.graphics.drawCircle(70, 70, 70);
		...
		icon.x = (bg.width - icon.width) / 2 + 25;
		icon.y = (bg.height - icon.height) / 2 + 93;
		
		nameItem.x = (bg.width - text.width) / 2 - 20;
		nameItem.y = 30;
		
		text.x = (bg.width - text.width) / 2 + 20;
		text.y = bg.height + 124;
