package wins {
	...
	public class TravelRequireWindow extends Window {
		...
		protected function drawRequire():void {
			...
			if (App.data.storage[sIDmap].hasOwnProperty('charge')) {
				enough = true;
				have = 0;
				need = 0;
				X = 0;
				for (req in App.data.storage[sIDmap].charge) {
					have = App.user.stock.data[req] || 0;
					need = App.data.storage[sIDmap].charge[req] || 1;
		...
		protected function onMove(e:MouseEvent):void {
			if (App.user.stock.checkAll(App.data.storage[sIDmap].charge)) {
				App.user.stock.takeAll(App.data.storage[sIDmap].charge);
