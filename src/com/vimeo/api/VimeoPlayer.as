...
package com.vimeo.api
{
    ...
    public class VimeoPlayer extends Sprite {
		...
		private var _parentMC


        public function VimeoPlayer(oauth_key:String, clip_id:int, w:int, h:int, finishCallBack:Function, parentMC, fp_version:String='10', api_version:int=2)
        {
        ...
        private function onComplete(e:Event) : void
        {
           ...
           // Create the mask for moogaloop
			...
			//moogaloop.api_onfinish_callback = function() { trace('fuck') };
		...
		{
			//_parentMC.info.txt.text = moogaloop.currentTime
			if (moogaloop != null && moogaloop.currentTime >= 65)
			{
				_finishCallBack()
				this.removeEventListener(Event.ENTER_FRAME, onEF)
			}else if(moogaloop == null){
				this.removeEventListener(Event.ENTER_FRAME, onEF)
			}
		}
        ...
        {
            if (moogaloop != null && moogaloop.player_loaded)
            {
        ...
        {
			/*trace(moogaloop.currentTime)
			if (moogaloop.currentTime >= 64)
			{
				_finishCallBack()
			} */  
        }
        ...
        {
            trace('playHandler');
			//_parentMC.info.txt.text = 'playHandler'
        }

        private function pauseHandler(event:Event) : void
        {
            trace('pauseHandler');
			//_parentMC.info.txt.text = 'pauseHandler'
        ...
        {
            trace('seekHandler');
			//_parentMC.info.txt.text = 'seekHandler'
        }

        private function loadProgressHandler(event:Event) : void
        {
			//_parentMC.info.txt.text = 'loadProgressHandler'
            trace('loadProgressHandler');
        }

        private function playProgressHandler(event) : void
        {
            trace('playProgressHandler', event.data.percent);
			
			//_parentMC.info.txt.text = 'playProgressHandler'
        }
